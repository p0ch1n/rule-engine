...
from rule_execution_engine.nodes import RelationNode
...
class TestRelationNode:
    ...
    def test_self_join_no_self_pairs(self):
        ...
        b = make_obj(x=0, y=0, w=10, h=10)
        result = node.execute({"input": [b]})
        assert len(result["output"]) == 0

    def test_cross_join(self):
        node = RelationNode(make_relation_config(mode="cross_join", threshold=0.1))
        b1 = make_obj(x=0, y=0, w=10, h=10, cls="person")
        b2 = make_obj(x=5, y=0, w=10, h=10, cls="car")
    ...
    def test_offset_applied(self):
        """Offset should shift the output relation obj."""
        node = RelationNode(
        ...
        result = node.execute({"input": [b1, b2]})
        rel = result["output"][0]
        # Union obj x=0, after dx=10 → x=10
    ...
    def test_distance_mode(self):
        ...
        b1 = make_obj(x=0, y=0, w=0, h=0)
        b2 = make_obj(x=3, y=4, w=0, h=0)  # dist = 5
        b3 = make_obj(x=100, y=100, w=0, h=0)  # dist ≈ 141
