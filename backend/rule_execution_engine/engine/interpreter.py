...
from rule_execution_engine.nodes import NodeRegistry  # triggers all @register decorators
from rule_execution_engine.nodes.base import BaseNode
from rule_execution_engine.schema.models import EdgeConfig, PipelineConfig
from rule_execution_engine.schema.validator import validate_pipeline
...
class Pipeline:
    ...
    def execute_frame(
        self,
        input_objects: Optional[List[Any]] = None,
        images: Optional[List[np.ndarray]] = None,
    ) -> "ExecutionResult":
        """Process a single frame through the pipeline.

        Args:
            input_objects: Pre-detected Object list for obj-only pipelines.
        ...
        """
        from rule_execution_engine.engine.scheduler import Scheduler

        scheduler = Scheduler(self._nodes, self._edges)
        return scheduler.run(input_objects=input_objects, images=images)
